...
const subLink = computed(() => {
  ...
  if (selectedId.value === 'default') {
    ...
    const url = `${baseUrl}/${token}/${selectedId.value}`;
...
const copyToClipboard = async () => {
  if (!subLink.value) {
    if (selectedId.value !== 'default' && (!props.config?.profileToken || props.config.profileToken === 'auto' || !props.config.profileToken.trim())) {
      showToast('请先在设置中配置"订阅组分享Token"', 'error');
    } else {
  ...
  <div class="sticky top-24">
    <div class="bg-white/60 dark:bg-gray-800/75 rounded-2xl border border-gray-300/50 dark:border-gray-700/30 p-6 relative overflow-hidden hover:shadow-xl transition-all duration-300">
      ...
      <div class="relative z-10">
        ...
        <!-- 提示信息 -->
        <div v-if="(selectedId === 'default' && config?.mytoken === 'auto') || (selectedId !== 'default' && (!config?.profileToken || config.profileToken === 'auto' || !config.profileToken.trim()))" class="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border-2 border-yellow-200 dark:border-yellow-800 animate-pulse-breathing">
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-sm font-medium text-yellow-700 dark:text-yellow-300 mb-1 animate-pulse">
                {{ selectedId === 'default' ? '自动Token提示' : '订阅组Token未配置' }}
              </p>
              <p class="text-xs text-yellow-600 dark:text-yellow-400">
                {{ selectedId === 'default' 
                  ? '当前为自动Token，链接可能会变化。为确保链接稳定，推荐在"设置"中配置一个固定Token。'
                  : '订阅组链接需要配置"订阅组分享Token"。请在"设置"中配置一个固定的"订阅组分享Token"后，才能生成有效的订阅组链接。' }}
              </p>
            </div>
          </div>
        </div>
